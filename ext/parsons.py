...
class ParsonsDirective(SphinxDirective):
    ...
    def run(self):
        ...
        if not(len(content_node) > 0 and isinstance(content_node[-1], nodes.literal_block)):
        ...
        parsonsnode.source, parsonsnode.line = self.state_machine.get_source_and_line(self.lineno)
...
def visit_parsonsitem (self, node):
    self.body.append('<div class="parsons-item" draggable="true" id="{id}" data-value="{a}" data-indent="{b}">\n'.format(a=node["data-value"], b=node["data-indent"], id=node["ids"][0]))
